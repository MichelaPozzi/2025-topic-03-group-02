...
for (protein in rownames(Shift_Data_for_t)) {
  ...
  if (all(!is.na(shift_values))) {
    ...
    # Nur wenn normalverteilt, den t-Test durchführen
    if (Shift_Data[protein, "normal_distributed"] == TRUE) {
      
      test_result <- t.test(shift_values, mu = 0, alternative = "two.sided")
      ...
      # Wenn nicht normalverteilt: kein t-Test
...
# Übersicht der Normalverteilungen
table(Shift_Data$normal_distributed, useNA = "ifany")

# Übersicht der Signifikanz nach t-Test
