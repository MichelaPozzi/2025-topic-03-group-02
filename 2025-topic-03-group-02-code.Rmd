...
glue("RNA-binding/-interacting proteins (UniProt) analyzed but not detected in our data: {length(not_indentified)}")

# Analyze potential causes
sum(is.na(Shift_Data[not_identified, "shapiro_p"])) /length(not_identified) # Missing Shapiro results because of missing values (approx.%)
shapiro_failed <- Shift_Data[is.na(Shift_Data$shapiro_p),]
glue("Number of Proteins, that where excluded, because shapiro results where NA: {nrow(shapiro_failed)}")

sum(Shift_Data[not_identified, "normal_distributed"] == FALSE) / length(not_identified) # Failed normality (approx. %)
normality_failed <- Shift_Data[Shift_Data$normal_distributed == FALSE,]
glue("Number of Proteins, that where excluded, because repliactes wheren't normally ditributed: {nrow(normality_failed)}")
