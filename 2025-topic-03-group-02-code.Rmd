...
#Normalisation - Cihan, Sofia,Lina
### Goal: 
...
- for less variance between the fractions --> This allows comparison of distribution shapes independent of absolute abundance
```{r}
...
# Shift Characteristics - Lina, Sofia, Chian
...
### Goal:
- All dimensions of our data, such as multiple peak positions and heights, are simplified for later t-tests (only one value) by using Center of Mass (CoM) values. The CoM is calculated for all replicates and treatments and data comes from Norm_Data_for_t (normalised Data). 

-It represents the weighted average position of a signal: the sum of all fraction numbers multiplied by their intensity values, divided by the total intensity. (This way, if a peak is "split" into two in one experiment but not in another, it should not affect the result significantly, as both would have similar CoM values. ) => was in klammern bin ich mir nicht sicher 
```{r}
# Setup
fractions <- 1:25
conditions <- c("Ctrl", "RNase")
...
for (cond in conditions) {
  for (rep in replicates) {
    ...
    cols <- grep(pattern, colnames(Norm_Data_for_t), value = TRUE) # chooses pattern from coloums (matching)
...
```
## Plot Protein Fuction with all additional Data

```{r}
plot_protein_info <- function(x) {
  
  #Split up plot area
  layout(matrix(1:2, nrow = 1), widths = c(5, 2.3))  # 3:2 Verhältnis (Plot : Info)
  ...
  #Core plot function using crtl_values
  plot(fractions, ctrl_values, type="o", pch=20, lty=1, lwd = 1.5,  col="forestgreen", ylim= ylim_range,
       xlab="Fraction", ylab="Normalized Intensity", main = NULL, axes = FALSE)
  
  # Titel
  mtext(protein_of_interest, side = 3, line = 1, adj = 0, cex = 1, font = 2)
  ...
  #Compute max peak postion in Ctrl and RNase
  
  p_data <- Peak_Data [x,]
  peak_heights_RNase <- as.numeric(p_data[, paste0("peak", 1:6, "_height_RNase")])
  peak_positions_RNase <- as.numeric(p_data[, paste0("peak", 1:6, "_position_RNase")])
  peak_heights_Ctrl <- as.numeric(p_data[, paste0("peak", 1:6, "_height_Ctrl")])
  peak_positions_Ctrl <- as.numeric(p_data[, paste0("peak", 1:6, "_position_Ctrl")])
  
  max_peak_height_RNase <- max(peak_heights_RNase, na.rm = TRUE)
  max_peak_height_Ctrl <- max(peak_heights_Ctrl, na.rm = TRUE)
  
  if (all(is.na(peak_heights_RNase))) {
    max_peak_position_RNase <- NA
    } 
  else {
    max_idx <- which.max(peak_heights_RNase)
    max_peak_position_RNase <- peak_positions_RNase[max_idx]
  }
  
  if (all(is.na(peak_heights_Ctrl))) {
    max_peak_position_Ctrl <- NA
    } 
  else {
    max_idx <- which.max(peak_heights_Ctrl)
    max_peak_position_Ctrl <- peak_positions_Ctrl[max_idx]
  }
  ...
  info_lines <- c(
    paste("Max Peak Position Ctrl:", max_peak_position_Ctrl),
    paste("Max Peak Height Ctrl:", round(max_peak_height_Ctrl, 1), "%"),
    paste("Max Peak Position RNase:", max_peak_position_RNase),
    paste("Max Peak Height RNase:", round(max_peak_height_RNase,1), "%"),
    ...
    paste("p-value:",round(shift_info$p_value_ttest_filtered_mitosis,4))
  )
  line_height <- 0.06
...
Manual inspection of 40 non-identified RBPs suggests a false negative rate of approximately 12% within the testable subset (visual shift despite test results) 
...
shapiro_NA <- Shift_Data[is.na(Shift_Data$shapiro_p),]
nrow(shapiro_NA)

sum(Shift_Data[not_indentified, "normal_distributed"] == FALSE) / length(not_indentified) # Failed normality
not_normally_dist <- Shift_Data[Shift_Data$normal_distributed == FALSE,]
nrow(not_normally_dist)
...
ggplot(Shift_Data, aes(x = `CoM_Ctrl`, y = `CoM_RNase`)) +
    ...
    "Overlaping with UniProt" = "darkblue",
...
plot(fit, fills = c("lightcoral", "#009ACD"), quantities = list(cex = 2), labels = FALSE)
```

## T-Test for non-synchronised HELA Cells - includes previous coded parts of all group members, slightly adjusted - Lina
...
# 3)looking at plots of different clusters (aus Linas code) 
...
# 4) finding the right parameters to choose the perfect cluster size 
...
GOAl: Build Datframe with Annotations for further analyses such as testing for RNA-dependent complexes
or predicting molecular weight from peak position.
...
# Background info — total number of unique proteins and complexes in CORUM
...
plot_protein_info <- function(x) {
  ...
  info_lines <- c(
    paste("Maximal Peak Position Ctrl:", max_peak_position_Ctrl),
    paste("Maximal Peak Height Ctrl:", max_peak_height_Ctrl),
    paste("Maximal Peak Position RNase:", max_peak_position_RNase),
    paste("Maximal Peak Height RNase:", max_peak_height_RNase),
    paste("CoM Ctrl:",round(shift_info$CoM_Ctrl_mitosis, 1)),
    paste("CoM RNase:", round(shift_info$CoM_RNase_mitosis, 1)),
    paste("Shift Distance:", round(shift_info$shift_distance_mitosis, 1)),
    paste("Identified as RBP:", shift_info$significant_ttest_filtered_mitosis),
...
GOAL: Investigate whether a linear model predicting molecular weight based on RNase values of this method (maximal peak position)can predict the molecular weight of proteins after RNase treatment.
...
for (cond in conditions) {
  for (rep in replicates) {
    ...
    # Single regex pattern to match all 25 fraction columns for this condition & replicate
    pattern <- paste0("^Fraction[0-9]+_", cond, "_", rep, "$")
    cols <- grep(pattern, colnames(Norm_Data_for_t), value = TRUE)
    ...
    # Center of mass calculation
    com <- apply(subdata, 1, function(x) sum(x * fractions) / sum(x))
