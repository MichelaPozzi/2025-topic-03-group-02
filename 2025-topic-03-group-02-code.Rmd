...
plot_protein <- function(x) {
  ...
  legend("right", legend=c("Ctrl", "RNase"), col=c("forestgreen", "firebrick3"), 
         lty=1, lwd = 1.5,  pch=20, bg = "white", bty = "o", box.col = NA)
}

plot_protein("ANM7_HUMAN") #fill in protein that you wanna see
```
```{r}
library(ggplot2)
library(dplyr)
...
plot_protein_gg <- function(x) {
  ...
  df <- data.frame(
    ...
    Intensity = c(ctrl_values, rnase_values),
    Condition = rep(c("Ctrl", "RNase"), each = 25)
  )

  ggplot(df, aes(x = Fraction, y = Intensity, color = Condition, fill = Condition)) +
    geom_line(size = 1) +
    geom_point(shape = 20, size = 4) +
    geom_ribbon(aes(ymin = 0, ymax = Intensity), alpha = 0.1, color = NA) +
    scale_color_manual(values = c("Ctrl" = "forestgreen", "RNase" = "firebrick3")) +
    scale_fill_manual(values = c("Ctrl" = "forestgreen", "RNase" = "firebrick3")) +
    ...
    geom_vline(xintercept = c(5, 10, 15, 20, 25),
               linetype = "dotted", color = "lightgray") 
}
plot_protein_gg("ANM7_HUMAN")
```
## Reproducibility Analysis : Cihan and Sofia 
- Goal: Assess how consistent our results are and how much variability comes from our data. For this, correlation is calculated for all combinations of fractions across all Ctrl and RNase samples and all repetitions. Logically, the diagonal shows a correlation with p-value = 1. Additionally, we expect a correlation of 1 for the same Ctrl/RNase condition and the same fraction across different repetitions. For the rest of the combinations, we expect very low correlation values. We used Spearman correlation for less sensitivity to outlyers. ( ist nicht der punkt von reproduzability auf variability zu testen, bin hier etwas confused). 
```{r}
...
db <- dbscan(scaled_data, eps = 0.7, minPts = 4)
...
# View Examples
sample_RBPs_Cluster <- c("RL22_HUMAN", "RL23_HUMAN", "RL24_HUMAN", "RL27_HUMAN", "RL30_HUMAN", "RL34_HUMAN", "RLA1_HUMAN", "RS26_HUMAN", "RS2_HUMAN", "RS6_HUMAN", "RS12_HUMAN")
...
target_proteins <- c("RS12_HUMAN", "RS26_HUMAN", "RS2_HUMAN", "RS6_HUMAN")
negative_controls <- c("LPPRC_HUMAN", "UIMC1_HUMAN")  # Add more if needed

all_proteins <- c(target_proteins, negative_controls)

# --- Step 3: create all pairwise combinations ---
