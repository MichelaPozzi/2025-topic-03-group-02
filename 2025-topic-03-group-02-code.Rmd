...
```{r}
# Example: Data is already loaded in df
# Row names are protein names
# Columns are named like: "F1_R1", "F1_R2", ..., "F25_R3"
# Choose your protein of interest
protein_name <- "1433B_HUMAN"

# Subset just the row of interest
protein_row <- data[protein_name, ]

# Initialize vector to store SDs
sd_values <- numeric(25)

# Get the three replicate values for this fraction from long-format data
for (i in 1:25) {
  # Filter the data for the specific protein and this fraction
  subset_df <- df[df$fraction == i & rownames(df) == protein_name, ]
  
  # Further filter for the 3 replicates: Ctrl_Rep1, Ctrl_Rep2, Ctrl_Rep3
  reps <- paste0("Ctrl_Rep", 1:3)
  values <- subset_df[subset_df$replicate %in% reps, "rownames"]  # Replace 'value_column' with your actual measurement column name
