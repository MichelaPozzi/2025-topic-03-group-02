...
# Step 1: your target proteins
target_proteins <- c("AURKA", "KIFC1", "TPX2")  # Replace with real names

# Step 2: all unique protein pairs
protein_pairs <- combn(target_proteins, 2, simplify = FALSE)

# Step 3: Initialize score storage
score_results <- data.frame()

# Step 4: Loop through each parameter combo (column)
for (col_name in colnames(results_df)) {
  cluster_assignments <- results_df[[col_name]]
...
ggplot(score_results, aes(x = eps, y = minPts, fill = score)) +
  ...
  labs(title = "Pairwise Clustering Consistency Score",
       subtitle = "Across DBSCAN parameter settings",
       x = "Epsilon (ε)",
       y = "MinPts",
       fill = "Avg. Score") +
  theme_minimal()
