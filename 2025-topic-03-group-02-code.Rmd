...
## Für alle Proteine auf Normalverteilung getestet

# Log-Transformation
MS_Table_Averages_Log <- log(MS_Table_Averages + 1)
# Ergebnis-DataFrame für Shapiro-Wilk-Test anlegen
shapiro_results_ctrl_log <- data.frame(
  Protein_ID = rownames(MS_Table_Averages_Log),
  Shapiro_W = NA,
  Shapiro_p = NA
)

# Spaltennamen für die Ctrl-Fraktionen extrahieren
ctrl_fraction_columns_log <- grep("^Ctrl_", colnames(MS_Table_Averages_Log), value = TRUE)

# Schleife über alle Proteine in MS_Table_Averages_Log
for (protein_index in 1:nrow(MS_Table_Averages_Log)) {
  
  # Aktuelles Protein bestimmen
  current_protein <- rownames(MS_Table_Averages_Log)[protein_index]
  
  # Log-transformierte Werte der Ctrl-Fraktionen für dieses Protein extrahieren
  current_ctrl_values_log <- as.numeric(MS_Table_Averages_Log[current_protein, ctrl_fraction_columns_log])
  
  # Prüfen ob alle Werte gleich sind
  if (length(unique(current_ctrl_values_log)) > 1) {
    # Nur wenn Varianz vorhanden, Shapiro-Test durchführen
    shapiro_test_result <- shapiro.test(current_ctrl_values_log)
    shapiro_results_ctrl_log$Shapiro_W[protein_index] <- shapiro_test_result$statistic
    shapiro_results_ctrl_log$Shapiro_p[protein_index] <- shapiro_test_result$p.value
  } else {
    # Wenn alle Werte gleich sind → NA setzen
    shapiro_results_ctrl_log$Shapiro_W[protein_index] <- NA
    shapiro_results_ctrl_log$Shapiro_p[protein_index] <- NA
  }
}

# Ergebnisse anzeigen
head(shapiro_results_ctrl_log)

# Schwellenwert für Normalverteilung definieren (p >= 0.05)
shapiro_results_ctrl_log$Is_Normal_Distributed <- shapiro_results_ctrl_log$Shapiro_p >= 0.05

# Übersicht, wie viele Proteine als normalverteilt eingestuft werden
table(shapiro_results_ctrl_log$Is_Normal_Distributed)
...
# Step 1: Make MS_Table long format (protein, rowname, abundance)
long_MS <- MS_Table %>%
  as.data.frame() %>%
...
# Step 2: Add protein column to your existing metadata table
data_all_proteins <- data %>%
  left_join(long_MS, by = "rownames")  # join on 'rownames'

# Step 3: View the new data table
print(data_all_proteins)

#Here starts the actual pearson calculation

# Step 1: Filter only relevant data
filtered <- data_all_proteins %>%
  filter(fraction >= 1, fraction <= 25,
         treatment %in% c("Ctrl", "RNAse"),
         replicate %in% c("_Rep_1", "2"))  # only Rep_1 and 2

# Step 2: Make sure Rep_1 and 2 are in separate columns
wide_data <- filtered %>%
  pivot_wider(
...
# Step 3: Calculate Pearson correlation between Rep_1 and 2
results <- wide_data %>%
  group_by(protein, treatment) %>%
  summarise(correlation = cor(`_Rep_1`, `2`), .groups = "drop")

# Step 4: Show results
print(results)
```

#trying to get Pearson values for a correlation 
...
results <- filtered %>%
  ...
  summarise(correlation = cor(Rep1, Rep2), .groups = "drop")
