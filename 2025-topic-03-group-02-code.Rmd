...
for (treatment in c("Ctrl", "RNAse")) {
  ...
  for (i in 1:nrow(combo1)) {
    for (j in 1:nrow(combo2)) {
      ...
      cor_value <- cor(MS_Table[, col1], MS_Table[, col2], method = "pearson", use = "complete.obs")
...
heatmap_data <- full_correlation_results %>%
  ...
  select(Row, Col, Pearson_r)

# In Matrix umwandeln
heatmap_matrix <- heatmap_data %>%
  pivot_wider(names_from = Col, values_from = Pearson_r, values_fill = NA) %>%
  column_to_rownames("Row") %>%
  as.matrix()

# Jetzt Heatmap zeichnen
pheatmap(heatmap_matrix,
         main = paste("Pearson Correlation Heatmap -", treatment_to_plot),
         cluster_rows = FALSE, 
         cluster_cols = FALSE, 
         show_rownames = TRUE, 
         show_colnames = TRUE)
